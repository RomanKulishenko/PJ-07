...
export class Comments {
  ...
  public addMockToLocalStorage(): void {
    fetch("../comments.json")
      ...
      .then((data) => {
        ...
        const commentsFromLocalStorage: Comment[] = JSON.parse(
  ...
  public showComments(
    ...
    const commentsWrap: HTMLElement = document.getElementById(
      "comments"
    ) as HTMLElement;
    ...
    if (comments !== null) {
      ...
      if (isFavorites) {
        setTimeout(() => {
          const commentsElements: NodeListOf<Element> =
          ...
          commentIds.forEach((commentId) => {
            const storedComment = JSON.parse(
      ...
      parsedComments.forEach(function (comment: MyComment): void {
        if (isFavorites) {
          ...
          if (comment.isReply) {
            setTimeout(() => {
              ...
              el.insertAdjacentHTML(
                "afterend",
                new Comment(comment).render(isFavorites)
              );
            }, 100);
          } else {
            commentsWrap.innerHTML += new Comment(comment).render(isFavorites);
  ...
  private addHandlersToButtons(): void {
    const commentsHTML: NodeListOf<Element> =
      document.querySelectorAll(".comment");
    commentsHTML.forEach((comment) => {
      const replyBtn: HTMLElement = comment.querySelector(
        ".comment__button--answer"
      ) as HTMLElement;
      const favoriteBtn: HTMLElement = comment.querySelector(
        ".comment__button--favorite"
      ) as HTMLElement;
      const ratingBtn: NodeListOf<Element> = comment.querySelectorAll(
        ".comment__button-rate"
      );
  ...
  public sort(comments: Comment[], sort: string, ascending: boolean = true) {
    ...
    return comments.sort((a, b) => {
      if (ascending) {
        ...
        if (sort === "replies") {
          return a.counterReplies - b.counterReplies;
        }
      } else {
        if (sort === "date") {
        ...
        if (sort === "replies") {
          return b.counterReplies - a.counterReplies;
        }
      }
      return 0;
    });
  ...
  private toggleFavorite = (event: Event): void => {
    this.getCommentHTMLElement(event);
    const favoriteTextHTML: HTMLElement = (
    ...
    if ((this.target as HTMLElement).classList.contains("active")) {
      ...
      this.updateFavoriteInLocalStorage(
        (this.targetParent as HTMLElement).id,
        false
      );
    } else {
  ...
  private calcRating = (event: Event): void => {
    ...
    const numHTML: HTMLElement = (
      this.targetParent as HTMLElement
    ).querySelector(".comment__button-num") as HTMLElement;
    ...
    if (ratingBtn.classList.contains("comment__button-rate--minus")) {
      ...
      this.updateRatingInLocalStorage(
        (this.targetParent as HTMLElement).id,
        currentRatingCalc
      );
    }
